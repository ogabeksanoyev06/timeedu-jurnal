  ...
  <div>
    ...
    <div class="container grid lg:grid-cols-12 gap-6">
      <div class="lg:col-span-8">
        ...
        <div class="grid md:grid-cols-12 gap-6 group" v-for="item in archiveData?.issues?.content" :key="item.id">
          <div class="md:col-span-3">
            <img :src="item.image" :alt="item.name" class="w-full object-contain rounded-xl" />
          </div>
          <div class="md:col-span-9">
            <NuxtLink :to="localePath(`/journal/${route.params.journalSlug}/currentIssue/${item.id}`)" class="mb-4 block group-hover:text-primary text-neutral font-medium text-xl">
              {{ archiveData?.name }}
            </NuxtLink>

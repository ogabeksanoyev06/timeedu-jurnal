<template>
  <div class="flex-y-center">
    <label class="group flex items-start select-none" :class="[disabled ? 'cursor-not-allowed' : 'cursor-pointer', wrapperClass,]">
      <input v-bind="{ disabled, checked: modelValue, indeterminate }" type="checkbox" class="absolute opacity-0 invisible h-0 w-0 peer" :value="modelValue" :name="name" @change="handleChange" />
      <span
        ...
        :class="[
          {
            ...
            '!border-primary !bg-primary after:opacity-100': modelValue,
            'border-gray-2': !modelValue,
            'bg-gray-100': !modelValue,
            'peer-checked:!border-primary peer-checked:!bg-primary': modelValue,
          },
          checkboxStyles,
        ]"
      />
